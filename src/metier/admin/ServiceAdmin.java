...
public class ServiceAdmin extends ServiceAuth implements IServiceAdmin, InteractiveConsole {
	...
	public List<Compte> trierComptesParNomPropriétaire() {
		...
		Collections.sort(comptes, (compte1, compte2) -> compte1.getPropriétaire().getNomComplet()
				.compareTo(compte2.getPropriétaire().getNomComplet()));
		return comptes;
	}
	...
	public TableauDeBord calculerEtAfficherStatistiques() {
		...
		tableauDeBord.setNomClientLePlusRiche(
				trierClientParSoldeCompte().get(trierClientParSoldeCompte().size() - 1).getNomComplet());
